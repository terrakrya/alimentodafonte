<template>
  <div>  
    <cool-select :arrowsDisableInstantSelection="true" placeholder="Busque pelo nome clique para adicionar à lista" v-model="seed" :items="items" item-text="title" @select="addItem()">
  ...
  methods: {
    addItem () {
      if (!this.form[this.field].find(item => (item.target_id 
        == this.seed.id))) {
        this.form[this.field].push({ target_id: this.seed.id })
