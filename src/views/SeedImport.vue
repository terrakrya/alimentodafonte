								...
								if (sheets['A'+i]) {
									...
									months.forEach((month, index) => {
										...
										var mes = this.meses.find(mes => {
											// console.log(mes.text.toLowerCase(), month);
									...
									if (months.length == 1) {
										...
										if (months[0] < months[1]) {
											for (var m = months[0]; m <= months[1]; m++) {
												fruiting_season.push(m + '')
											}
										} else {
											for (var m = months[0]; m <= 12; m++) {
												fruiting_season.push(m + '')
											}
											for (var m = 1; m <= months[1]; m++) {
												fruiting_season.push(m + '')
											}
										}
									}
			...
			this.import_preview.forEach((item, index) => {
				...
				axios.get('seeds/search', { params: { name: item.name } }).then(resp => {
					...
					axios({
						...
						if (seed && seed._id) {
							if (isEditing) {
								this.import_log.push({status: 'success', message: (index + ' - ' + seed.name + ': atualizada com sucesso'), index: index})
							} else {
								this.import_log.push({status: 'success', message: (index + ' - ' + seed.name + ': cadastrada com sucesso'), index: index})
							}
						} else {
							this.import_log.push({status: 'success', message: (index + ' - Erro: '+seed), index: index})
							this.error = seed
						}
