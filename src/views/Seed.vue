	...
	<div class="dashboard">
		...
		<div class="panel panel-headline data-list">
			<div class="panel-body">
				...
				<div class="row" v-if="seed && !loading">
					<div class="col-md-12">
						<div class="item-title">
							<div class="fish-img" v-if="seed.field_images && seed.field_images.length">
								<img v-bind:src="seed.field_images[0].url" />
							</div>
							<h1>
								{{ seed.title[0].value }}
							</h1>
					...
					<div class="col-md-12">
						<hr>
						<div class="row">
							<div class="col-md-3" v-if="seed_variation.price && seed_variation.price.length">
								<div class="weekly-summary text-center">
									<span class="info-label">Preço</span>
									<span class="number">{{ seed_variation.price[0].number | currency('R$ ', 2, { decimalSeparator: ',' }) }}</span>
								</div>
							</div>
							<div class="col-md-3" v-if="seed_variation.field_wholesale_price && seed_variation.field_wholesale_price.length">
								<div class="weekly-summary text-center">
									<span class="info-label">Preço no atacado</span>
									<span class="number">{{ seed_variation.field_wholesale_price[0].number | currency('R$ ', 2, { decimalSeparator: ',' }) }}</span>
								</div>
							</div>
							<div class="col-md-3" v-if="seed.field_compensation_collect && seed.field_compensation_collect.length">
								<div class="weekly-summary text-center">
									<span class="info-label">Remuneração do coletor</span>
									<span class="number">{{ seed.field_compensation_collect[0].number | currency('R$ ', 2, { decimalSeparator: ',' }) }}</span>
								</div>
							</div>
							<div class="col-md-3" v-if="seed_variation.field_stock && seed_variation.field_stock.length">
								<div class="weekly-summary text-center">
									<span class="info-label">Quantidade em estoque</span>
									<span class="number">{{ seed_variation.field_stock[0].value | currency('', 0) }}</span>
								</div>
							</div>
						</div>
							...
							<tr>
								<td width="50%" valign="top">
									<dl>
										...
										<dd>{{ seed.field_seeds_kg[0].value | currency('', 0) }} sementes / Kg</dd>
									</dl>
									<dl>
										<dt>Taxa de viabilidade</dt>
										<dd>{{ seed.field_viability_rate[0].value | currency('', 0) }} % de viabilidade</dd>
									</dl>
									<dl>
										<dt>Limite de peso por lote</dt>
										<dd>{{ seed.field_lot_limit[0].value | currency('', 0) }} kg por lote</dd>
									</dl>
								</td>
								<td width="50%" valign="top">
